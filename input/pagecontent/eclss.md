...
* **ch4_ppm** (float, ppm) – Methane concentration in cabin air. Normally very low (a few ppm). CH₄ can be produced by human metabolism in trace amounts or released from experiments/hardware. Relevance: Methane is not highly toxic at trace levels but is monitored as part of atmospheric quality. It is also a flammability concern if concentrated (though nominal levels are far below the lower flammable limit). On ISS, the Major Constituent Analyzer tracks CH₄ continuously. An unusual rise could indicate a leak from a system or an unexpected biologic process.  
  ...
  _Spacecraft notes:_ All modern spacecraft have the capability to detect elevated methane as a safety metric, though nominal readings are near-zero. This field would typically read zero or a small value under normal conditions.  
...
* **h2_ppm** (float, ppm) – Hydrogen concentration in cabin air. Nominally ~0 ppm; any detected H₂ is significant. Relevance: Hydrogen is a highly flammable gas, monitored to detect leaks (e.g. from fuel cells on Shuttle, water electrolysis equipment on ISS). Even small H₂ accumulations are taken seriously; readings approaching a fraction of a percent would prompt alarms due to fire/explosion risk.  
  ...
  _Spacecraft notes:_ ISS’s analyzer monitors H₂, and Apollo/Shuttle had H₂ sensors (fuel cell byproduct). This field may often be zero; a non-zero reading triggers troubleshooting. Units in ppm (or µatm partial pressure) allow detecting tiny increases.  
...
| Limit | Partial pressure | Approximate concentration | Source |
|-------|------------------|---------------------------|--------|
...
The JSON field names on this page are a telemetry vocabulary, not FHIR resources, but the guide already carries the same measurements in FHIR form. The [Spacecraft Life Support Telemetry](StructureDefinition-SpacecraftLifeSupportTelemetry.html) logical model mirrors this page field-for-field in camelCase (`o2Percent`, `co2Ppm`, `cabinPressureKPa`, `radiationDoseRateUSvPerHr`, and so on), grouped into the same atmospheric, climate, contaminant, particulate, and radiation sections, so it is the formal definition of the schema described here. When a reading is exchanged as a clinical or operational record it is carried as an Observation conforming to the [Environmental Monitoring](StructureDefinition-environmental-monitoring.html) profile, with the field name mapping to `Observation.code` (or to `Observation.component.code` when several readings are reported together) and the value and UCUM unit carried in `valueQuantity`. The radiation fields (`radiation_dose_rate_uSv_per_hr`, `radiation_cumulative_mSv`, `shielding_mode`) correspond to the [Space Radiation Exposure](StructureDefinition-space-radiation-exposure.html) and [Cumulative Radiation Dose](StructureDefinition-cumulative-radiation-dose.html) profiles and the [Shielding Mass](StructureDefinition-shielding-mass.html) extension described on the [Radiation Tracking](radiation-tracking.html) page. A related logical model for closed-biome habitats and greenhouses, [Environmental Monitoring API for Closed Biome Systems](StructureDefinition-EnvironmentalMonitoringLogicalModel.html), reuses the atmospheric subset of these fields; see the [Biospheres](biomes.html) page.
