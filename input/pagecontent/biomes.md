...
The JSON field list above is not itself a FHIR artifact; it is the sensor vocabulary that the guide's environmental profiles are built from. The same fields, grouped by the same categories (solar, thermal mass, atmospheric life support, HVAC and pressure control, biome health, external environment), are expressed as the [Environmental Monitoring API for Closed Biome Systems](StructureDefinition-EnvironmentalMonitoringLogicalModel.html) logical model, where each JSON key becomes a decimal element with the unit carried in its description. Spacecraft-specific parameters (cabin partial pressures, trace contaminants, particulates, radiation) are modeled separately in the [Spacecraft Life Support Telemetry](StructureDefinition-SpacecraftLifeSupportTelemetry.html) logical model described on the [ECLSS page](eclss.html). When a habitat reports an actual reading rather than describing its schema, it uses the [Environmental Monitoring](StructureDefinition-environmental-monitoring.html) Observation profile, with one Observation (or one component per sensor) carrying the value as a UCUM quantity; [Observation - Environmental](Observation-Observation.Environmental-1.html) is a minimal synthetic example. Implementers who already publish the JSON API can therefore map a field such as `co2_ppm` to an Observation component whose value is a Quantity in `[ppm]`, and a field such as `co2_ppm_avg` to a second component or to a separate Observation with an `effectivePeriod`. No new profiles are proposed on this page.
