# Fitness - Aerospace Medicine Implementation Guide v0.7.0
...
| | | |
| :--- | :--- | :--- |
| [SpaceExerciseActivityMeasure](StructureDefinition-space-exercise-activity-measure.md) | Individual exercise measurements | Duration, intensity, heart rate, power output, modality-specific metrics |
| [SpaceExerciseActivityGroup](StructureDefinition-space-exercise-activity-group.md) | Panel grouping related measures | Links type, duration, intensity for a single session; mission context |
| [SpaceEVSMinutesPerWeek](StructureDefinition-space-evs-minutes-per-week.md) | Weekly exercise volume summary | EVS (Exercise Vital Sign) minutes, aggregated across modalities |
| [SpaceExerciseSession](StructureDefinition-space-exercise-session.md) | Procedural record of exercise event | Links to prescriptions, goals, devices, and observation panels |
...
The shared extensions used on these profiles are [MissionContext](StructureDefinition-mission-context.md), [ExerciseDevice](StructureDefinition-exercise-device.md), [HarnessLoad](StructureDefinition-harness-load.md) and [DerivedFromGroup](StructureDefinition-derived-from-group.md).
...
Venous flow and hemostasis profiles are the concern of the [Cardiovascular Countermeasures](cardiovascular-countermeasures.md) page, which defines the IJV flow, pressure, area and Flow Directionality Index profiles.
...
* **[SpaceExerciseModalityCS](CodeSystem-space-exercise-modality-cs.md)**: treadmill, cycle-ergometer, ARED-resistive, resistive-squat, resistive-deadlift, ag-centrifuge
* **[SpaceExerciseModalityVS](ValueSet-space-exercise-modality-vs.md)**: ValueSet for space exercise modalities
* **[SpacePerformanceMetricCS](CodeSystem-space-performance-metric-cs.md)**: mean-power, peak-power, simulated-bw, total-work, vo2-surrogate, recovery-hr
* **[SpacePerformanceMetricVS](ValueSet-space-performance-metric-vs.md)**: ValueSet for exercise performance metrics
* **[EVSUnitsVS](ValueSet-evs-units-vs.md)**: Exercise Vital Sign (EVS) unit measurements
...
Venous flow and coagulation codes (IJV flow velocity, flow stasis, thrombin generation, hypercoagulability) belong with the [Cardiovascular Countermeasures](cardiovascular-countermeasures.md) page; [VenousFlowMetricCS](CodeSystem-venous-flow-metric-cs.md) already covers the flow-directionality component codes.
...
* **Observation** — [SpaceExerciseActivityMeasure](StructureDefinition-space-exercise-activity-measure.md), [SpaceExerciseActivityGroup](StructureDefinition-space-exercise-activity-group.md), [SpaceEVSMinutesPerWeek](StructureDefinition-space-evs-minutes-per-week.md)
* **Procedure** — [SpaceExerciseSession](StructureDefinition-space-exercise-session.md)
...
* [MissionContext](StructureDefinition-mission-context.md) — reference to the mission Encounter (consistent with the radiation, nutrition, and neutral-buoyancy modules)
* [ExerciseDevice](StructureDefinition-exercise-device.md) — reference to the specific equipment used
* [HarnessLoad](StructureDefinition-harness-load.md) — percentage of body weight applied for treadmill sessions
* [DerivedFromGroup](StructureDefinition-derived-from-group.md) — links an EVS summary to its constituent activity groups
...
The guide carries a worked set of synthetic instances for a single 30-minute ISS treadmill session during Expedition 75: the [in-flight encounter](Encounter-ISS-Exp75-InFlight.md) and the [T2 treadmill device](Device-ISS-T2-Treadmill.md), three individual measures — [activity type](Observation-ISS-T2-Run-Type-001.md), [duration](Observation-ISS-T2-Run-Duration-001.md) and [mean heart rate](Observation-ISS-T2-Run-MeanHR-001.md) — the [session panel](Observation-ISS-T2-Run-Group-001.md) that groups them, the [weekly EVS summary](Observation-ISS-EVS-MinPerWeek-2025W23.md), and the [session procedure](Procedure-ISS-Exercise-Session-Proc-001.md).

The two JSON blocks below show the measure and the panel inline.

#### Example 1: SpaceExerciseActivityMeasure - Mean Heart Rate

An individual measure carries two codings: one from an established terminology (LOINC here, or the Physical Activity IG temporary codes for measures such as duration, which LOINC does not cover well) and one from [SpaceExerciseModalityCS](CodeSystem-space-exercise-modality-cs.md) saying which piece of space hardware produced it.
