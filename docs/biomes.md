# Biospheres - Aerospace Medicine Implementation Guide v0.7.0
...
Closed biome habitats – whether Earth-based smart greenhouses, orbital stations, or lunar and Martian colonies – require comprehensive real-time monitoring of environmental conditions. A JSON-based API for such a system should expose all key life-support and diagnostic variables needed to maintain a stable, habitable environment. This includes monitoring solar radiation for plant growth, thermal storage for temperature control, atmospheric composition (for example CO₂, O₂, humidity, and pressure) for life support, HVAC parameters (including an external "lung" expansion chamber for pressure regulation), and overall biome health indicators (water pH, ozone levels, and so on). Each sensor reading can be reported as a current value along with aggregated averages to aid trend analysis and system control.

The design draws on the sealed-habitat experience of Biosphere 2 and the Space Analog for the Moon and Mars (SAM) test module at the University of Arizona, where crews watched CO₂ level, wet-bulb temperature (for humidity), ozone concentration, pH, and even the position of the external "lung" on a single control panel, each reading shown as a current value ("Now") alongside a running average ("Avg"). Such comprehensive monitoring is crucial to maintaining life-supporting conditions in closed systems. The following sections outline sensor data categories, recommended JSON fields (with data types and units), and the purpose of each field in a closed-biome or space habitat context. The same field list is expressed as a FHIR logical model in this guide; see [Relationship to FHIR Profiles](#relationship-to-fhir-profiles) below.

### Solar Input/Output Measurements

Closed ecosystems often rely on sunlight (or artificial light) for plant photosynthesis and heat. **Photosynthetically Active Radiation (PAR)** sensors at different heights help ensure light penetrates the plant canopy evenly, and solar radiation sensors track overall energy input. These fields inform lighting control (for example supplemental LED lights or shades) and thermal management.
...
| | | |
| :--- | :--- | :--- |
| concrete_loop_in_temp_C | float | Inlet water temperature (°C) into the concrete thermal mass loop. Paired with the outlet reading below, it gives the temperature drop or rise across the slab. |
...
The atmospheric conditions inside the biome must be kept within narrow ranges for human and plant life. Key gas concentrations, pressure, temperature, and humidity are continuously monitored. These fields mirror those tracked in spacecraft life support systems (the ISS Environmental Control and Life Support System, ECLSS, monitors O₂, N₂, CO₂, H₂O, and other constituents) and in smart greenhouses.

| | | |
| :--- | :--- | :--- |
| co2_ppm | float | Carbon dioxide concentration in air (parts per million). Critical for life support: high CO₂ impairs crew cognition and plant respiration. This value is actively managed via CO₂ scrubbers or plant uptake – without removal, CO₂ can accumulate to dangerous levels. (NASA's ISS operational limit is a 24-hour average of 3 mmHg, about 4,000 ppm, with a 7-day Spacecraft Maximum Allowable Concentration of 5.3 mmHg, about 7,000 ppm, per JSC 20584; see the[ECLSS page](eclss.md)for the full threshold table.) |
...
| ozone_ppm | float | Ozone concentration in the air (ppm). Ozone (O₃) is a harmful oxidant; even low levels can irritate lungs and damage plant tissues. In a closed habitat, ozone might be produced by electrical equipment or used in water sterilization, so it must be monitored to remain safe (typically near 0 ppm). For reference, U.S. airworthiness rules (14 CFR 25.832) limit transport-aircraft cabin ozone to 0.25 ppm by volume at any time above 32,000 ft, and to a 0.1 ppm time-weighted average over any 3-hour interval above 27,000 ft. An elevated ozone_ppm reading would trigger scrubbing or filtering because ozone above safe levels can degrade air quality and materials. |
...
**Data Grouping and Access Patterns.** The JSON structure can be organized to improve clarity and integration. One approach is to group fields into nested objects by category or physical zone. The following block is illustrative of that grouping rather than a complete message:
...
The JSON field list above is not itself a FHIR artifact; it is the sensor vocabulary that the guide's environmental profiles are built from. The same fields, grouped by the same categories (solar, thermal mass, atmospheric life support, HVAC and pressure control, biome health, external environment), are expressed as the [Environmental Monitoring API for Closed Biome Systems](StructureDefinition-EnvironmentalMonitoringLogicalModel.md) logical model, where each JSON key becomes a decimal element with the unit carried in its description. Spacecraft-specific parameters (cabin partial pressures, trace contaminants, particulates, radiation) are modeled separately in the [Spacecraft Life Support Telemetry](StructureDefinition-SpacecraftLifeSupportTelemetry.md) logical model described on the [ECLSS page](eclss.md). When a habitat reports an actual reading rather than describing its schema, it uses the [Environmental Monitoring](StructureDefinition-environmental-monitoring.md) Observation profile, with one Observation (or one component per sensor) carrying the value as a UCUM quantity; [Observation - Environmental](Observation-Observation.Environmental-1.md) is a minimal synthetic example. Implementers who already publish the JSON API can therefore map a field such as `co2_ppm` to an Observation component whose value is a Quantity in `[ppm]`, and a field such as `co2_ppm_avg` to a second component or to a separate Observation with an `effectivePeriod`. No new profiles are proposed on this page.

### References

* Aranet. [The Impact of Fotoniq's PAR+ Coating and Aranet Sensors](https://pro.aranet.com/the-impact-of-fotoniqs-par-coating-and-aranet-sensors/) – case study on greenhouse PAR distribution.
* The Emerald Review. [Biosphere 2: The Futuristic Space Colony That Wasn't](https://emeraldreview.com/2024/08/biosphere-2-the-futuristic-space-colony-that-never-was/), 2024 – Biosphere 2 lung mechanism for pressure regulation.
* University of Arizona News. [Becoming a multiplanetary species: Crew completes first mission in pressurized habitat at Biosphere 2](https://news.arizona.edu/news/becoming-multiplanetary-species-crew-completes-first-mission-pressurized-habitat-biosphere-2) – SAM habitat test monitoring (lung pressure, O₂/CO₂ levels).
* Ceres Greenhouse Solutions. [Tips on Using Water Barrels in a Solar Greenhouse](https://ceresgs.com/tips-on-using-water-barrels-in-a-solar-greenhouse/) – water as thermal mass.
* NASA. [International Space Station (ISS) Environmental Control and Life Support (ECLS) System Overview of Events: 2010–2014](https://ntrs.nasa.gov/api/citations/20150022318/downloads/20150022318.pdf) – monitored atmospheric constituents (O₂, N₂, CO₂, H₂O, etc.).
* NASA Office of the Chief Health and Medical Officer. [Environmental Control & Life Support System (ECLSS) technical brief](https://www.nasa.gov/wp-content/uploads/2023/07/eclss-technical-brief-ochmo.pdf).
* NASA. [Spacecraft Maximum Allowable Concentrations for Airborne Contaminants, JSC 20584 Rev C](https://www.nasa.gov/wp-content/uploads/2024/06/jsc-20584-smacs-rev-c-final.pdf) – CO₂ operational limits and SMACs.
* American Chemical Society. [Dual Function Materials Enabling Human Space Flight: Carbon Dioxide Capture and Conversion for Life Support on Crewed Missions](https://pmc.ncbi.nlm.nih.gov/articles/PMC11955851/), 2025 – CO₂ removal is critical to prevent dangerous buildup in spacecraft.
* Federal Aviation Administration. Airworthiness Standards: Transport Category Airplanes, Cabin ozone concentration, [14 CFR 25.832](https://www.ecfr.gov/current/title-14/part-25/section-25.832) – 0.25 ppm cabin ozone limit.
* Embry-Riddle Aeronautical University Scholarly Commons. [An Analysis of Cabin Ozone Regulations](https://commons.erau.edu/cgi/viewcontent.cgi?article=2125&context=publication).
* Atlas Scientific. [Why is pH Important for Hydroponics?](https://atlas-scientific.com/blog/ph-for-hydroponics/) – optimal nutrient solution pH (~5.5–6.5) for plant nutrient uptake.
